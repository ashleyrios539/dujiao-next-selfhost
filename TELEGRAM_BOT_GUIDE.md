...
> **随时可退出**：在购买/充值/卡头库存等任何「文本输入」步骤里，发送 `/cancel`（取消并清空会话）、或 `/start`、`/menu`、`/help`（清空会话后跳到对应页面）即可退出当前流程；卡头库存、充值金额的入口提示也带「❌ 取消」按钮，点击同样退出。
...
/shop
   ...
   ▼
选择分类 ──> 选择商品 ──> 商品初始页
   │  文本：随机购买/挑头购买各显示「毛料(不测活) 包活(测活)」两档价格（实时取网页端配置）
   │  8 个购买类型按钮（各带 [库存] + 毛料价）：
   │    [随机购买]  [挑头购买]   ← 大按钮
   │    [3头 库存xx 金额] [4头 库存xx 金额] [5头 库存xx 金额] [6头 库存xx 金额]
   │    [CREDIT 库存xx 金额] [DEBIT 库存xx 金额]
   │  头 = 卡号首位数字（3头=3开头、4头=4开头…）；DEBIT=借记卡(含预付，提交D)、CREDIT=信用卡(提交C)
   ▼
挑头购买 → 直接输入 6 位 BIN（无需国家）
其余类型 → 选国家（🇺🇸US emoji+双字母，按库存降序，一页6个，可翻页，也可回复双字母代码）
   ▼
【测活选择】（仅商品开启测活时出现）
   reply 键盘（输入框弹出）两个按钮：[毛料] [包活]，点一下即发送文字
   （未开启测活的商品跳过这步直接进确认页）
   ▼
订单确认（展示用户选择 + 付款金额）
   ▼
...
- 挑卡与测活**复用网页商城的计价与校验逻辑**（后端 `order` 模块），订单快照（`PickCountry`/`PickBrands`/`PickCardTypes`/`PickBin`/`CardCheckEnabled`）可在网页个人中心查看。
- **3头/4头/5头/6头首位挑卡**：按卡号首位匹配库存（`bin_prefix LIKE 'N%'`），加价取对应 `head3/head4/head5/head6` 加价项（管理端可独立配置）；下单提交 `PickBin=首位`+`PickCountry=国家`。
- **DEBIT/CREDIT**：DEBIT 提交卡类型 `D`（在库存查询里匹配 D+PD，覆盖所有借记卡），加价取 `pick_prices["D"]`；CREDIT 提交 `C`，加价取 `pick_prices["C"]`。
- 国家选择：点击内联键盘中的国家按钮（emoji+双字母，按库存降序，一页6个带翻页），或**直接回复双字母国家代码**（如 `US`）。
- 挑头(BIN)购买不需要选国家；其余类型必须选国家。
- **测活**用 reply 键盘（输入框弹出键盘）选择毛料/包活，点一下即发送文字（非消息上的 inline 按钮）。
