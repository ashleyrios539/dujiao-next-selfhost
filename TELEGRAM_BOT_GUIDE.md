...
| 命令 | 行为 |
|---|---|
| `/start` | 发送欢迎语（后台配置，可多语言）；未配置欢迎语时只发主菜单提示 |
| `/shop` | **进入在线购买**：在聊天内浏览分类→商品→选 SKU/数量→输入挑头 BIN→勾选测活→确认下单→余额或在线支付 |
...
/shop
   ...
   ▼
选择分类 ──> 选择商品 ──> 选 SKU / 调整数量
   │                     ├─ 支持挑头：直接输入 6 位 BIN，实时显示该 BIN 可用库存
   │                     └─ 支持测活：点击「开启/关闭测活」开关
   ▼
订单确认（展示单价 / 测活费 / 挑头加价 / 合计）
   ▼
余额支付 或 在线支付（复用网页商城同一套支付与钱包逻辑）
```

- 挑头(BIN)与测活均**复用网页商城的计价与校验逻辑**（后端 `order` 模块），同一订单快照（`PickBin`/`CardCheckEnabled`）可在网页个人中心查看。
...
| Key | 默认标签（zh-CN） | 含义 |
|---|---|---|
| `shop_home` | 🛍️ 开始购物 | 进入商城 |
| `my_orders` | 📦 我的订单 | 查看订单 |
| `my_wallet` | 💰 我的钱包 | 查看钱包 |
| `affiliate` | 📣 推广返利 | 分销推广 |
| `gift_card` | 🎁 礼品卡兑换 | 兑换礼品卡 |
| `switch_language` | 🌐 切换语言 | 切换语言 |
