...
- **语言切换**：中/英双向切换（当前中文显示「English」，英文显示「中文」），偏好写入商城账号（`users.locale`），bot 与网页共用。
- **充值**：`/recharge` 或「我的钱包」结果中的「💳 充值」按钮 → 输入金额 → 选渠道（USDT/Epusdt 等）→ 生成收银台链接，支付后**自动到账**（复用既有支付回调链路）。
- **在线支付（epusdt 直连）**：购买确认页点「在线支付」后，**直接在聊天内**显示应付 USDT 金额、TRC20 收款地址（代码块，**点按即复制**）、网络与过期时间，并附带**收款地址二维码**图片，无需跳转收银台；支付成功后自动发货，卡密以 **txt 文件**推送到本对话。付款页带「🔄 刷新支付状态」「📦 我的订单」「🏠 返回主页」按钮。
- **充值（epusdt 直连）**：`/recharge`（或钱包页「💳 充值」）→ 输入 **USDT 金额**（提示语明确为「请输入充值 USDT 金额」）→ 聊天内直接显示应付 USDT + 收款地址（可点按复制）+ 二维码，支付后自动到账。付款页展示后会话即结束，**再次输入数字不会重复创建充值订单**，输入其他内容自动回退主菜单；付款页带「🔄 查看到账状态」「🏠 返回主页」按钮。
...
- 上限 12 条 FAQ，每条可配置多语言「摘要 / 标题 / 正文」；
- 默认内置 4 条：怎么下单、订单问题、钱包充值、联系客服；
- 条目可配置 `show_support_link`（在详情里附带客服链接，即基础设置里的支持链接）。
