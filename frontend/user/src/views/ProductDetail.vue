                ...
                <div class="space-y-5">
                  ...
                  <div v-if="pickMode === 'random' || pickMode === 'type'">
                    ...
                    <div v-if="pickCountry" class="mb-2 inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-sm">
                      <span class="font-medium text-primary">{{ selectedCountryName }}</span>
                    ...
                    <div v-else class="relative">
                      <input
                        ...
                        @blur="onCountryBlur"
                      />
                      <div v-if="countryDropdownOpen" class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-xl border border-input bg-background shadow-lg">
                        <button
                        ...
                        <p v-if="filteredCountries.length === 0" class="px-3 py-2 text-sm text-muted-foreground">—</p>
                      </div>
                    </div>
                  ...
                  <div v-if="pickMode === 'bin'">
                    <label class="mb-2 block text-sm font-medium text-foreground">{{ t('productDetail.pickBinLabel') }}</label>
                    <input
                      v-model="pickBin"
                      type="text"
                      inputmode="numeric"
                      maxlength="6"
                      class="w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm font-mono tracking-widest outline-none transition-colors focus:border-primary"
                      :placeholder="t('productDetail.pickBinLabel')"
                    />
                    <p v-if="binStockLoading" class="mt-1.5 text-xs text-muted-foreground">{{ t('productDetail.pickStockLoading') }}</p>
                    <p v-else-if="pickBin.length === 6 && /^\d{6}$/.test(pickBin)" class="mt-1.5 text-xs" :class="(binStockCount ?? 0) > 0 ? 'text-emerald-600' : 'text-destructive'">
                  ...
                  <template v-if="pickMode === 'type'">
                    <div>
                      <label class="mb-2 block text-sm font-medium text-foreground">{{ t('productDetail.pickHeadLabel') }}</label>
                      <div class="flex flex-wrap gap-2">
                        <button
                      ...
                      <p v-if="headStockLoading" class="mt-1.5 text-xs text-muted-foreground">{{ t('productDetail.pickStockLoading') }}</p>
                      <p v-else-if="pickHead && pickHead !== 'random' && (headStockCount ?? 0) === 0" class="mt-1.5 text-xs text-destructive">
                        {{ t('productDetail.pickHeadAvailable', { count: headStockCount ?? 0 }) }}
                      </p>
                      <p v-else-if="pickHead && pickHead !== 'random'" class="mt-1.5 text-xs text-emerald-600">
                        {{ t('productDetail.pickHeadAvailable', { count: headStockCount ?? 0 }) }}
                      </p>
                    </div>
                    <div>
                    ...
                    <p v-else-if="pickCountry && !pickHead && !pickCardTypes.length" class="text-xs text-amber-500">
                      {{ t('productDetail.pickTypeRequired') }}
                    </p>
                  </template>

                  <!-- 当前所选 -->
                  <div v-if="pickMode" class="rounded-lg bg-muted/60 px-3.5 py-2.5 text-sm">
                  ...
                  <!-- 加价提示（最终单价在下方测活区统一展示） -->
                  <p v-if="pickUnitSurcharge > 0" class="text-xs text-muted-foreground">
              ...
              <div v-if="product.card_check_enabled" class="mb-8">
                ...
                <div class="mt-3 flex flex-wrap items-end gap-x-8 gap-y-2">
                  <div>
                    ...
                    <div class="theme-price-lg" :class="cardCheckEnabled ? 'text-muted-foreground' : 'text-primary'">{{ formatPrice(finalPlainPrice, siteCurrency) }}</div>
                  </div>
                  <div>
                    <div class="text-xs text-muted-foreground">{{ t('productDetail.cardCheckCheckedPrice') }}</div>
                    <div class="theme-price-lg" :class="cardCheckEnabled ? 'text-primary' : 'text-muted-foreground'">{{ formatPrice(finalCheckedPrice, siteCurrency) }}</div>
                  </div>
                </div>
              </div>
