...
挑卡依赖 **BIN 库**给卡密自动标注属性（国家/品牌/种类）。启用流程：

### 1. 上传 BIN 库（推荐：管理后台）

后台 → **卡密管理 → BIN 库** → **上传 BIN 库**，选择你的 BIN 库 CSV。默认列映射适配常见表头：
...
- **显式映射**（默认 `CREDIT→C`、`CHARGE→C`）：命中直接得到对应种类。
- **预付标记**（默认 `PREPAID`，在预付标记列 `Category` 中匹配）：未命中显式映射的非信用卡，标记列命中 → `D`（含预付），未命中 → `纯D`（不含预付）。

> 品牌自动归一化为 `visa / mastercard / discover / other`。上传后新导入的卡密会自动标注；未命中的卡属性留空，仅作普通卡售卖。

### 2. 按商品开启"支持挑卡"并设置加价

管理后台 → 商品 → 编辑，勾选 **支持挑卡** 并设置各属性加价单价（Visa / Mastercard / Discover / 其他 / D卡 / 纯D / 纯C）。同一属性组（品牌/种类）多选时只按该组**最高单价**计一次，加价并入商品单价参与优惠计算。

### 3. 用户端行为

- 商品页：开启挑卡的商品会显示**国家下拉（必选）**，可继续选择是否按品牌/种类挑卡；实时显示"该组合可发 N 张"与挑卡单价。
- 下单/预留：按所选组合从库存过滤出卡；**组合不足直接报库存不足，无法下单**。
- 交付：测活与不测活两条路径都按订单项快照过滤，交付的卡一定满足用户所选国家/品牌/种类。
